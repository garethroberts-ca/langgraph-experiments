...
# (Banner printing moved into main() to avoid side effects on import)
...
# ============================================================
# These tools allow agents to navigate and control graph execution

from langgraph.types import Send, Command, interrupt

# Global reference to hold compiled graph (set after build)
_COMPILED_GRAPH = None
_GRAPH_BUILDER = None


def set_graph_reference(compiled_graph, builder=None):
    """Set the global graph reference for traversal tools."""
    global _COMPILED_GRAPH, _GRAPH_BUILDER
    _COMPILED_GRAPH = compiled_graph
    _GRAPH_BUILDER = builder


@tool
def get_graph_state(thread_id: str) -> str:
    """Get the current state of a graph execution thread.
    
    Args:
        thread_id: The thread ID to inspect
    
    Returns:
        JSON string with current state snapshot
    """
    if _COMPILED_GRAPH is None:
        return "Error: Graph not initialized. Call set_graph_reference() first."
    
    try:
        config = {"configurable": {"thread_id": thread_id}}
        state = _COMPILED_GRAPH.get_state(config)
        return json.dumps({
            "values": {k: str(v)[:200] for k, v in (state.values or {}).items()},
            "next": state.next,
...
def list_graph_nodes() -> str:
    """List all available nodes in the graph.
    
    Returns:
        JSON array of node names
    """
    if _COMPILED_GRAPH is None:
...
@tool
def get_state_history(thread_id: str, limit: int = 5) -> str:
    ...
    try:
        config = {"configurable": {"thread_id": thread_id}}
        history = []
        for state in _COMPILED_GRAPH.get_state_history(config):
            history.append({
                "next": state.next,
                "created_at": state.created_at,
                "tasks": [t.name for t in state.tasks] if state.tasks else []
...
@tool
def update_graph_state(thread_id: str, updates: str, as_node: str = "__input__") -> str:
    """Update the state of a graph execution thread.
    
    Args:
        thread_id: The thread ID to update
        updates: JSON string of state updates (e.g., '{"route": "feedback"}')
    ...
    try:
        ...
        _COMPILED_GRAPH.update_state(config, update_dict, as_node=as_node)
        return f" State updated for thread {thread_id}: {list(update_dict.keys())}"
...
@tool
def create_graph_checkpoint(thread_id: str, note: str = "") -> str:
    """Get checkpoint info for a thread (useful for debugging).
    
    Args:
        thread_id: The thread ID
        note: Optional note about this checkpoint
...
# Helper functions for programmatic navigation (not tools, for internal use)

def create_send_to_node(node_name: str, state_update: dict) -> Send:
    """Create a Send object to route to a specific node with custom state.
    
    Args:
        node_name: Target node name
        state_update: State to pass to the node
    
    Returns:
        Send object for use in conditional edges
    """
    return Send(node_name, state_update)


def create_command(goto: str | list = None, update: dict = None, resume: Any = None) -> Command:
    """Create a Command object for graph control.
    
    Args:
        goto: Node(s) to navigate to
    ...
    """
    return Command(
        goto=goto if goto else (),
        update=update,
        resume=resume
    )


def request_human_input(prompt: str) -> Any:
    """Request human input by interrupting the graph.
    
    Args:
        prompt: Message to show the user
    
    Returns:
        The value provided when resuming
    """
    return interrupt(prompt)


# Traversal tools for LLM agents
TRAVERSAL_TOOLS = [get_graph_state, list_graph_nodes, get_state_history, update_graph_state, create_graph_checkpoint]

# Combined tools list (action + traversal)
ALL_TOOLS = TOOLS + TRAVERSAL_TOOLS
...
def visualize_graph(
    output_path: str = "hr_platform_graph",
    format: str = "png",
    show: bool = True,
    include_legend: bool = True,
    layout: str = "dot"
) -> str:
    """Create a network visualization of the HR Platform graph.
    
    Args:
        output_path: Base filename for output (without extension)
        format: Output format ('png', 'svg', 'pdf', 'html')
        show: Whether to open the visualization after creating
    ...
    """
    if _COMPILED_GRAPH is None:
        raise RuntimeError("Graph not initialized. Call build_ultimate_platform() first.")
    
    # Try using LangGraph's built-in visualization first
    try:
        # LangGraph has a get_graph() method that returns a drawable graph
        graph_drawable = _COMPILED_GRAPH.get_graph()
        ...
        pass  # Fall back to custom visualization
    
    # Custom visualization using networkx and matplotlib
    try:
        import networkx as nx
        import matplotlib.pyplot as plt
        from matplotlib.patches import Patch
    except ImportError:
        raise ImportError("Please install networkx and matplotlib: pip install networkx matplotlib")
    ...
    node_categories = {
        "entry": {"color": "#4CAF50", "nodes": ["__start__", "classify", "retrieve"]},
        "specialist": {"color": "#2196F3", "nodes": [
            "specialist_career", "specialist_conflict", "specialist_wellbeing",
            "specialist_feedback", "specialist_leadership", "specialist_goals",
            "specialist_compensation", "specialist_general"
        ]},
        "perform": {"color": "#9C27B0", "nodes": [
        ...
        "quality": {"color": "#FF9800", "nodes": ["evaluate", "optimize", "finalize"]},
        "bulk": {"color": "#00BCD4", "nodes": ["parse_bulk", "process_chunk", "synthesize_bulk", "format_bulk"]},
        "promotion": {"color": "#E91E63", "nodes": ["promotion_analyze", "format_promotion"]},
        "planning": {"color": "#CDDC39", "nodes": ["orchestrator", "execute_action_worker", "synthesize_plan", "format_plan"]},
        "agent": {"color": "#FF5722", "nodes": ["agent_think", "agent_act", "agent_done"]},
        "end": {"color": "#607D8B", "nodes": ["__end__"]}
    }
    ...
    edges = [
        ...
        # Routing from retrieve (conditional - show as dashed)
        ("retrieve", "specialist_general"),
        ("retrieve", "specialist_career"),
        ("retrieve", "specialist_conflict"),
        ("retrieve", "specialist_wellbeing"),
        ("retrieve", "specialist_feedback"),
        ("retrieve", "specialist_leadership"),
        ("retrieve", "specialist_goals"),
        ("retrieve", "specialist_compensation"),
        ("retrieve", "perform_self_reflection"),
        ("retrieve", "perform_review_writing"),
        ("retrieve", "perform_goal_setting"),
        ("retrieve", "perform_one_on_one"),
        ("retrieve", "perform_feedback_request"),
        ("retrieve", "perform_feedback_writing"),
        ("retrieve", "perform_shoutout"),
        ("retrieve", "perform_calibration"),
        ("retrieve", "perform_competency"),
        ("retrieve", "parse_bulk"),
        ("retrieve", "promotion_analyze"),
        ("retrieve", "orchestrator"),
        ("retrieve", "agent_think"),
        ...
        ("format_promotion", "__end__"),
        
        # Action planning
        ("orchestrator", "execute_action_worker"),
        ...
        ("agent_act", "agent_think"),
        ("agent_act", "agent_done"),
        ("agent_done", "__end__"),
    ]
    ...
    if include_legend:
        legend_elements = [
            Patch(facecolor=info["color"], label=category.title())
            for category, info in node_categories.items()
        ]
        ax.legend(
    ...
    ax.set_title(
        "Culture Amp HR Platform - LangGraph Architecture",
        fontsize=16,
        fontweight="bold",
        pad=20
    )
    ...
    plt.savefig(full_path, format=format, dpi=150, bbox_inches="tight", facecolor="white")
...
def visualize_graph_mermaid(output_path: str = "hr_platform_graph.md") -> str:
    ...
    Returns:
        Mermaid diagram as a string (also saved to file)
    """
    mermaid = """```mermaid
graph TD
...
"""
    ...
    with open(output_path, "w") as f:
        f.write(f"# HR Platform Graph Architecture\n\n{mermaid}")
...
def visualize_subgraphs(
    ...
    """Visualize each subgraph of the HR Platform separately.
    ...
    Returns:
        Dict mapping subgraph names to their output file paths
    """
    try:
        import networkx as nx
        import matplotlib.pyplot as plt
    ...
    # Define subgraphs with their nodes, edges, and colors
    subgraphs = {
        "entry": {
            "title": "Entry Flow",
            "color": "#4CAF50",
            "nodes": ["__start__", "classify", "retrieve"],
            "edges": [
                ("__start__", "classify"),
                ("classify", "retrieve")
            ],
            "description": "Request enters, gets classified, knowledge retrieved"
        },
        "specialists": {
            "title": "Specialist Coaches (Routing)",
            "color": "#2196F3",
            "nodes": [
                "retrieve", "specialist_career", "specialist_conflict", "specialist_wellbeing",
                "specialist_feedback", "specialist_leadership", "specialist_goals",
                "specialist_compensation", "specialist_general", "evaluate"
            ],
            "edges": [
                ("retrieve", "specialist_career"), ("retrieve", "specialist_conflict"),
                ("retrieve", "specialist_wellbeing"), ("retrieve", "specialist_feedback"),
                ("retrieve", "specialist_leadership"), ("retrieve", "specialist_goals"),
                ("retrieve", "specialist_compensation"), ("retrieve", "specialist_general"),
                ("specialist_career", "evaluate"), ("specialist_conflict", "evaluate"),
                ("specialist_wellbeing", "evaluate"), ("specialist_feedback", "evaluate"),
                ("specialist_leadership", "evaluate"), ("specialist_goals", "evaluate"),
                ("specialist_compensation", "evaluate"), ("specialist_general", "evaluate")
            ],
            "description": "8 specialist coaches for different HR topics"
        },
        "perform": {
            "title": "Culture Amp Perform Features",
            "color": "#9C27B0",
            "nodes": [
                "retrieve", "perform_self_reflection", "perform_review_writing",
                "perform_goal_setting", "perform_one_on_one", "perform_feedback_request",
                "perform_feedback_writing", "perform_shoutout", "perform_calibration",
                "perform_competency", "evaluate"
            ],
            "edges": [
                ("retrieve", "perform_self_reflection"), ("retrieve", "perform_review_writing"),
                ("retrieve", "perform_goal_setting"), ("retrieve", "perform_one_on_one"),
                ("retrieve", "perform_feedback_request"), ("retrieve", "perform_feedback_writing"),
                ("retrieve", "perform_shoutout"), ("retrieve", "perform_calibration"),
                ("retrieve", "perform_competency"),
                ("perform_self_reflection", "evaluate"), ("perform_review_writing", "evaluate"),
                ("perform_goal_setting", "evaluate"), ("perform_one_on_one", "evaluate"),
                ("perform_feedback_request", "evaluate"), ("perform_feedback_writing", "evaluate"),
                ("perform_shoutout", "evaluate"), ("perform_calibration", "evaluate"),
                ("perform_competency", "evaluate")
            ],
            "description": "9 Culture Amp Perform feature handlers"
        },
        "quality": {
            "title": "Quality Control (Evaluator-Optimizer)",
            "color": "#FF9800",
            "nodes": ["evaluate", "optimize", "finalize", "__end__"],
            "edges": [
                ("evaluate", "optimize"), ("evaluate", "finalize"),
                ("optimize", "evaluate"), ("finalize", "__end__")
            ],
            "description": "Quality scoring loop with optimization"
        },
        "bulk": {
            "title": "Bulk Processing (Fan-out/Fan-in)",
            "color": "#00BCD4",
            "nodes": ["retrieve", "parse_bulk", "process_chunk", "synthesize_bulk", "format_bulk", "__end__"],
            "edges": [
                ("retrieve", "parse_bulk"), ("parse_bulk", "process_chunk"),
                ("process_chunk", "synthesize_bulk"), ("synthesize_bulk", "format_bulk"),
                ("format_bulk", "__end__")
            ],
            "description": "Parallel chunk processing with Send API"
        },
        "promotion": {
            "title": "Promotion Analysis (Parallelization + Voting)",
            "color": "#E91E63",
            "nodes": ["retrieve", "promotion_analyze", "format_promotion", "__end__"],
            "edges": [
                ("retrieve", "promotion_analyze"),
                ("promotion_analyze", "format_promotion"),
                ("format_promotion", "__end__")
            ],
            "description": "8 parallel analyzers + 5 voting perspectives"
        },
        "planning": {
            "title": "Action Planning (Orchestrator-Workers)",
            "color": "#CDDC39",
            "nodes": ["retrieve", "orchestrator", "execute_action_worker", "synthesize_plan", "format_plan", "__end__"],
            "edges": [
                ("retrieve", "orchestrator"), ("orchestrator", "execute_action_worker"),
                ("execute_action_worker", "synthesize_plan"), ("synthesize_plan", "format_plan"),
                ("format_plan", "__end__")
            ],
            "description": "Orchestrator delegates to specialized workers"
        },
        "agent": {
            "title": "Autonomous Agent (Think-Act Loop)",
            "color": "#FF5722",
            "nodes": ["retrieve", "agent_think", "agent_act", "agent_done", "__end__"],
            "edges": [
                ("retrieve", "agent_think"), ("agent_think", "agent_act"),
                ("agent_act", "agent_think"), ("agent_act", "agent_done"),
                ("agent_done", "__end__")
            ],
            "description": "ReAct-style reasoning loop for complex triage"
        }
    }
    ...
    for name, config in subgraphs.items():
        ...
        for node in G.nodes():
            ...
            elif node in ["retrieve", "classify"]:
                node_colors.append("#4CAF50")
            elif node == "evaluate":
                node_colors.append("#FF9800")
            else:
                node_colors.append(config["color"])
        ...
        nx.draw_networkx_edges(
            G, pos, edge_color="#666666", arrows=True, arrowsize=20,
            arrowstyle="-|>", connectionstyle="arc3,rad=0.1", alpha=0.7, ax=ax
        )
        
        ax.set_title(f"{config['title']}\n{config['description']}", fontsize=14, fontweight="bold", pad=20)
        ax.axis("off")
        
        plt.tight_layout()
        
        output_path = f"{output_dir}/subgraph_{name}.{format}"
        plt.savefig(output_path, format=format, dpi=150, bbox_inches="tight", facecolor="white")
...
def visualize_subgraphs_mermaid(output_path: str = "hr_platform_subgraphs.md") -> str:
    """Generate separate Mermaid diagrams for each subgraph.
    
    Args:
        output_path: Path for the markdown file
    
    Returns:
        Markdown content with all subgraph diagrams
    """
    content = """# HR Platform - Subgraph Architecture
...
graph TD
    ...
    class START,classify,retrieve entry
```
*Request enters, gets classified, knowledge retrieved*

---

## 2. Specialist Coaches (Routing Pattern)
```mermaid
graph TD
    retrieve --> specialist_career
    retrieve --> specialist_conflict
    ...
    class specialist_career,specialist_conflict,specialist_wellbeing,specialist_feedback,specialist_leadership,specialist_goals,specialist_compensation,specialist_general specialist
    class evaluate quality
```
*8 specialist coaches handle different HR topics*

---

## 3. Culture Amp Perform Features
```mermaid
graph TD
    retrieve --> perform_self_reflection
    retrieve --> perform_review_writing
    ...
    class evaluate quality
```
*9 Culture Amp Perform feature handlers*

---

## 4. Quality Control (Evaluator-Optimizer Pattern)
```mermaid
graph TD
    evaluate --> |score < 7| optimize
    evaluate --> |score >= 7| finalize
    optimize --> evaluate
    finalize --> END[__end__]
    ...
    class END endpoint
```
*Quality scoring loop with configurable threshold*

---

## 5. Bulk Processing (Fan-out/Fan-in)
```mermaid
graph TD
    retrieve --> |bulk request| parse_bulk
    parse_bulk --> |"Send()"| process_chunk1[process_chunk]
    parse_bulk --> |"Send()"| process_chunk2[process_chunk]
    parse_bulk --> |"Send()"| process_chunk3[process_chunk]
    process_chunk1 --> synthesize_bulk
    process_chunk2 --> synthesize_bulk
    process_chunk3 --> synthesize_bulk
    synthesize_bulk --> format_bulk
    format_bulk --> END[__end__]
    
    classDef entry fill:#4CAF50,stroke:#333,color:#fff
    classDef bulk fill:#00BCD4,stroke:#333,color:#fff
    classDef endpoint fill:#607D8B,stroke:#333,color:#fff
    class retrieve entry
    class parse_bulk,process_chunk1,process_chunk2,process_chunk3,synthesize_bulk,format_bulk bulk
    class END endpoint
```
*Parallel chunk processing with LangGraph Send API*

---

## 6. Promotion Analysis (Parallelization + Voting)
```mermaid
graph TD
    retrieve --> |promotion review| promotion_analyze
    
    subgraph promotion_analyze["promotion_analyze"]
        direction TB
        subgraph Analyzers["8 Parallel Analyzers"]
            technical[Technical]
            delivery[Delivery]
        ...
        subgraph Voters["5 Voting Perspectives"]
            advocate[Advocate]
            skeptic[Skeptic]
            manager[Manager]
            balanced[Balanced]
    ...
    class END endpoint
```
*8 parallel analyzers + 5 weighted voting perspectives*

---

## 7. Action Planning (Orchestrator-Workers Pattern)
```mermaid
graph TD
    retrieve --> |action plan| orchestrator
    
    orchestrator --> |"Send()"| worker1[Goal Worker]
    orchestrator --> |"Send()"| worker2[Timeline Worker]
    orchestrator --> |"Send()"| worker3[Resource Worker]
    orchestrator --> |"Send()"| worker4[Risk Worker]
    orchestrator --> |"Send()"| worker5[Milestone Worker]
    
    worker1 --> synthesize_plan
    worker2 --> synthesize_plan
    worker3 --> synthesize_plan
    worker4 --> synthesize_plan
    worker5 --> synthesize_plan
    
    synthesize_plan --> format_plan
    format_plan --> END[__end__]
    
    classDef entry fill:#4CAF50,stroke:#333,color:#fff
    classDef planning fill:#CDDC39,stroke:#333,color:#000
    classDef endpoint fill:#607D8B,stroke:#333,color:#fff
    class retrieve entry
    class orchestrator,worker1,worker2,worker3,worker4,worker5,synthesize_plan,format_plan planning
    class END endpoint
```
...
## 8. Autonomous Agent (Think-Act Loop)
```mermaid
graph TD
    retrieve --> |risk triage| agent_think
    agent_think --> |plan| agent_act
    agent_act --> |need more info| agent_think
    agent_act --> |complete| agent_done
    agent_done --> END[__end__]
    ...
    class END endpoint
```
*ReAct-style reasoning loop for complex triage*

---

## Summary: Anthropic Patterns Implemented

| Pattern | Implementation | Nodes |
|---------|----------------|-------|
| **Augmented LLM** | KnowledgeBase, Tools, Memory | classify, retrieve |
| **Routing** | 8 specialist coaches | specialist_* |
| **Parallelization** | ThreadPoolExecutor + voting | promotion_analyze |
| **Orchestrator-Workers** | Dynamic task delegation | orchestrator, workers |
| **Evaluator-Optimizer** | Quality scoring loop | evaluate, optimize |
| **Autonomous Agent** | Think-Act-Observe loop | agent_* |
"""
...
def visualize_graph_html(output_path: str = "hr_platform_graph.html", show: bool = True) -> str:
...
<body>
    <h1>🎯 Culture Amp HR Platform - Graph Architecture</h1>
    <div class="legend">
        <div class="legend-item"><div class="legend-color" style="background:#4CAF50"></div> Entry</div>
        <div class="legend-item"><div class="legend-color" style="background:#2196F3"></div> Specialists</div>
        <div class="legend-item"><div class="legend-color" style="background:#9C27B0"></div> Perform</div>
        <div class="legend-item"><div class="legend-color" style="background:#FF9800"></div> Quality</div>
        <div class="legend-item"><div class="legend-color" style="background:#00BCD4"></div> Bulk</div>
        <div class="legend-item"><div class="legend-color" style="background:#E91E63"></div> Promotion</div>
        <div class="legend-item"><div class="legend-color" style="background:#CDDC39"></div> Planning</div>
        <div class="legend-item"><div class="legend-color" style="background:#FF5722"></div> Agent</div>
    </div>
    <div id="graph"></div>
    <script>
        const nodes = new vis.DataSet([
            // Entry
            {id: "__start__", label: "START", color: "#4CAF50", shape: "diamond"},
            {id: "classify", label: "classify", color: "#4CAF50"},
            {id: "retrieve", label: "retrieve", color: "#4CAF50"},
            
            // Specialists
            {id: "specialist_general", label: "general", color: "#2196F3", group: "specialist"},
            {id: "specialist_career", label: "career", color: "#2196F3", group: "specialist"},
            {id: "specialist_conflict", label: "conflict", color: "#2196F3", group: "specialist"},
            {id: "specialist_wellbeing", label: "wellbeing", color: "#2196F3", group: "specialist"},
            {id: "specialist_feedback", label: "feedback", color: "#2196F3", group: "specialist"},
            {id: "specialist_leadership", label: "leadership", color: "#2196F3", group: "specialist"},
            {id: "specialist_goals", label: "goals", color: "#2196F3", group: "specialist"},
            {id: "specialist_compensation", label: "compensation", color: "#2196F3", group: "specialist"},
            
            // Perform
            {id: "perform_self_reflection", label: "self_reflection", color: "#9C27B0", group: "perform"},
            {id: "perform_review_writing", label: "review_writing", color: "#9C27B0", group: "perform"},
            {id: "perform_goal_setting", label: "goal_setting", color: "#9C27B0", group: "perform"},
            {id: "perform_one_on_one", label: "one_on_one", color: "#9C27B0", group: "perform"},
            {id: "perform_feedback_request", label: "feedback_request", color: "#9C27B0", group: "perform"},
            {id: "perform_feedback_writing", label: "feedback_writing", color: "#9C27B0", group: "perform"},
            {id: "perform_shoutout", label: "shoutout", color: "#9C27B0", group: "perform"},
            {id: "perform_calibration", label: "calibration", color: "#9C27B0", group: "perform"},
            {id: "perform_competency", label: "competency", color: "#9C27B0", group: "perform"},
            
            // Quality
            {id: "evaluate", label: "evaluate", color: "#FF9800"},
            {id: "optimize", label: "optimize", color: "#FF9800"},
            {id: "finalize", label: "finalize", color: "#FF9800"},
            
            // Bulk
            {id: "parse_bulk", label: "parse_bulk", color: "#00BCD4"},
            {id: "process_chunk", label: "process_chunk", color: "#00BCD4"},
            {id: "synthesize_bulk", label: "synthesize_bulk", color: "#00BCD4"},
            {id: "format_bulk", label: "format_bulk", color: "#00BCD4"},
            
            // Promotion
            {id: "promotion_analyze", label: "promotion_analyze", color: "#E91E63"},
            {id: "format_promotion", label: "format_promotion", color: "#E91E63"},
            
            // Planning
            {id: "orchestrator", label: "orchestrator", color: "#CDDC39"},
            {id: "execute_action_worker", label: "action_worker", color: "#CDDC39"},
            {id: "synthesize_plan", label: "synthesize_plan", color: "#CDDC39"},
            {id: "format_plan", label: "format_plan", color: "#CDDC39"},
            
            // Agent
            {id: "agent_think", label: "think", color: "#FF5722"},
            {id: "agent_act", label: "act", color: "#FF5722"},
            {id: "agent_done", label: "done", color: "#FF5722"},
            
            // End
            {id: "__end__", label: "END", color: "#607D8B", shape: "diamond"}
        ]);
        ...
        const edges = new vis.DataSet([
            ...
            {from: "retrieve", to: "specialist_general", dashes: true},
            {from: "retrieve", to: "specialist_career", dashes: true},
            {from: "retrieve", to: "specialist_conflict", dashes: true},
            {from: "retrieve", to: "specialist_wellbeing", dashes: true},
            {from: "retrieve", to: "specialist_feedback", dashes: true},
            {from: "retrieve", to: "specialist_leadership", dashes: true},
            {from: "retrieve", to: "specialist_goals", dashes: true},
            {from: "retrieve", to: "specialist_compensation", dashes: true},
            
            // Routing to perform
            {from: "retrieve", to: "perform_self_reflection", dashes: true},
            {from: "retrieve", to: "perform_review_writing", dashes: true},
            {from: "retrieve", to: "perform_goal_setting", dashes: true},
            {from: "retrieve", to: "perform_one_on_one", dashes: true},
            {from: "retrieve", to: "perform_feedback_request", dashes: true},
            {from: "retrieve", to: "perform_feedback_writing", dashes: true},
            {from: "retrieve", to: "perform_shoutout", dashes: true},
            {from: "retrieve", to: "perform_calibration", dashes: true},
            {from: "retrieve", to: "perform_competency", dashes: true},
            
            // Specialists to evaluate
            {from: "specialist_general", to: "evaluate"},
            {from: "specialist_career", to: "evaluate"},
            {from: "specialist_conflict", to: "evaluate"},
            {from: "specialist_wellbeing", to: "evaluate"},
            {from: "specialist_feedback", to: "evaluate"},
            {from: "specialist_leadership", to: "evaluate"},
            {from: "specialist_goals", to: "evaluate"},
            {from: "specialist_compensation", to: "evaluate"},
            
            // Perform to evaluate
            {from: "perform_self_reflection", to: "evaluate"},
            {from: "perform_review_writing", to: "evaluate"},
            {from: "perform_goal_setting", to: "evaluate"},
            {from: "perform_one_on_one", to: "evaluate"},
            {from: "perform_feedback_request", to: "evaluate"},
            {from: "perform_feedback_writing", to: "evaluate"},
            {from: "perform_shoutout", to: "evaluate"},
            {from: "perform_calibration", to: "evaluate"},
            {from: "perform_competency", to: "evaluate"},
            
            // Quality loop
            {from: "evaluate", to: "optimize", dashes: true, label: "needs work"},
            {from: "evaluate", to: "finalize", dashes: true, label: "good"},
            {from: "optimize", to: "evaluate"},
            {from: "finalize", to: "__end__"},
            
            // Bulk
            {from: "retrieve", to: "parse_bulk", dashes: true},
            {from: "parse_bulk", to: "process_chunk"},
            {from: "process_chunk", to: "synthesize_bulk"},
            {from: "synthesize_bulk", to: "format_bulk"},
            {from: "format_bulk", to: "__end__"},
            
            // Promotion
            {from: "retrieve", to: "promotion_analyze", dashes: true},
            {from: "promotion_analyze", to: "format_promotion"},
            {from: "format_promotion", to: "__end__"},
            
            // Planning
            {from: "retrieve", to: "orchestrator", dashes: true},
            {from: "orchestrator", to: "execute_action_worker"},
            {from: "execute_action_worker", to: "synthesize_plan"},
            {from: "synthesize_plan", to: "format_plan"},
            {from: "format_plan", to: "__end__"},
            
            // Agent
            {from: "retrieve", to: "agent_think", dashes: true},
            {from: "agent_think", to: "agent_act"},
            {from: "agent_act", to: "agent_think", dashes: true, label: "continue"},
            {from: "agent_act", to: "agent_done", dashes: true, label: "done"},
            {from: "agent_done", to: "__end__"}
        ]);
...
def create_default_state() -> dict:
    """Create a default state dictionary for the platform.
    
    Returns:
        Dict with all required state fields initialized to empty/default values
    """
    return {
        "messages": [],
        "request_type": "",
...
def build_ultimate_platform():
    ...
    # Register for traversal tools
    set_graph_reference(compiled, graph)
...
def main():
    """CLI entrypoint to run selected demos or list capabilities"""
...
""")
    ...
    def print_stats_if_any(result_state: dict):
        stats = result_state.get("processing_stats", {}) if isinstance(result_state, dict) else {}
        if stats:
            print("\n— Progress Summary —")
            try:
                if "elapsed" in stats:
                ...
                if "voting" in stats:
                    v = stats["voting"]
                    print(f"Voting: {v.get('completed', 0)}/{v.get('total', 0)} in {v.get('elapsed', 0):.1f}s")
    ...
    # --- Demo functions ---
    def demo_conversation(platform):
        print("\n" + "═" * 78)
        print("DEMO: CONVERSATION COACHING (Routing + Evaluator-Optimizer)")
        print("═" * 78)
        ...
        print_stats_if_any(result)

    def demo_promotion(platform):
        print("\n" + "═" * 78)
        print("DEMO: PROMOTION ANALYSIS (Parallel Sectioning + Voting)")
        print("═" * 78)
        promotion_feedback = """
...
"""
    ...
    def demo_action_plan(platform):
        print("\n" + "═" * 78)
        print("DEMO: ACTION PLANNING (Orchestrator-Workers Pattern)")
        print("═" * 78)
        config = {"configurable": {"thread_id": "demo-3"}}
        result = platform.invoke({
            "messages": [HumanMessage(content=(
                "I want to transition from Senior Engineer to Engineering Manager.\n"
    ...
    def demo_feedback_writing(platform):
        print("\n" + "═" * 78)
        print("DEMO: FEEDBACK IMPROVEMENT (SBI Model)")
        print("═" * 78)
        config = {"configurable": {"thread_id": "demo-4"}}
        result = platform.invoke({
            "messages": [HumanMessage(content=(
                "Help me improve this feedback I want to give:\n\n"
    ...
    # Culture Amp Perform demo set
    def demo_self_reflection(platform):
        config = {"configurable": {"thread_id": "demo-self-reflection"}}
        result = platform.invoke({
            "messages": [HumanMessage(content=(
                "I completed three major projects this quarter, improved deployment reliability, "
                "and mentored two new hires. I struggled with saying no and overcommitted twice."
            ))],
            "request_type": RequestType.SELF_REFLECTION.value,
            "request_complexity": "medium",
            "identified_topics": ["self_reflection"],
            "route": "self_reflection",
            "route_confidence": 0.9,
            "retrieved_knowledge": {},
            "memory_context": "",
            "chunks": [],
    ...
    def demo_goal_setting(platform):
        config = {"configurable": {"thread_id": "demo-goal-setting"}}
        result = platform.invoke({
            "messages": [HumanMessage(content=(
                "I want to improve cross-team collaboration and reduce incident MTTR by 30%."
            ))],
            "request_type": RequestType.GOAL_SETTING.value,
            "request_complexity": "medium",
            "identified_topics": ["goals"],
            "route": "goals",
            "route_confidence": 0.9,
            "retrieved_knowledge": {},
            "memory_context": "",
    ...
    def demo_one_on_one(platform):
        config = {"configurable": {"thread_id": "demo-1on1"}}
        result = platform.invoke({
            "messages": [HumanMessage(content=(
                "Next 1:1 with my manager: discuss growth into tech lead, align on Q1 goals, "
                "and get feedback on communication."
            ))],
            "request_type": RequestType.ONE_ON_ONE.value,
            "request_complexity": "low",
            "identified_topics": ["one_on_one"],
            "route": "one_on_one",
            "route_confidence": 0.9,
            "retrieved_knowledge": {},
            "memory_context": "",
    ...
    def demo_feedback_request(platform):
        config = {"configurable": {"thread_id": "demo-feedback-request"}}
        result = platform.invoke({
            "messages": [HumanMessage(content=(
                "I led the migration to a new CI pipeline. I want to ask my peers and manager "
                "for specific feedback to improve."
            ))],
            "request_type": RequestType.FEEDBACK_REQUEST.value,
            "request_complexity": "low",
            "identified_topics": ["feedback"],
    ...
    def demo_shoutout(platform):
        config = {"configurable": {"thread_id": "demo-shoutout"}}
        result = platform.invoke({
            "messages": [HumanMessage(content=(
                "I'd like to recognize Priya for leading the incident response last week with "
                "calm and clarity, living our 'Customer First' value."
            ))],
            "request_type": RequestType.SHOUTOUT.value,
            "request_complexity": "low",
            "identified_topics": ["shoutout"],
            "route": "shoutout",
            "route_confidence": 0.9,
            "retrieved_knowledge": {},
            "memory_context": "",
    ...
    def demo_calibration(platform):
        config = {"configurable": {"thread_id": "demo-calibration"}}
        result = platform.invoke({
            "messages": [HumanMessage(content=(
                "Employee shows strong delivery and mentoring, but scope is limited to team. "
                "Ratings vary between Performing and Exceeding across reviewers."
            ))],
            "request_type": RequestType.CALIBRATION.value,
            "request_complexity": "medium",
            "identified_topics": ["calibration"],
            "route": "calibration",
            "route_confidence": 0.9,
            "retrieved_knowledge": {},
            "memory_context": "",
    ...
    def demo_competency(platform):
        config = {"configurable": {"thread_id": "demo-competency"}}
        result = platform.invoke({
            "messages": [HumanMessage(content=(
                "Assess my competencies based on my last two quarters: delivery, mentorship, "
                "communication, and innovation contributions."
            ))],
            "request_type": RequestType.COMPETENCY_ASSESSMENT.value,
            "request_complexity": "medium",
            "identified_topics": ["competency"],
            "route": "competency",
            "route_confidence": 0.9,
            "retrieved_knowledge": {},
            "memory_context": "",
    ...
    # --- CLI ---
    parser = argparse.ArgumentParser(description="Ultimate HR Coaching Platform demos")
    group = parser.add_mutually_exclusive_group()
    group.add_argument("--list-demos", action="store_true", help="List available demos and exit")
    group.add_argument("--all", action="store_true", help="Run all demos")
    parser.add_argument("--demo", action="append", default=[], help="Run a specific demo by name (can be repeated)")
    parser.add_argument("--debug", action="store_true", help="Enable debug logging")
    parser.add_argument("--summary", action="store_true", help="Print capabilities summary at the end")
    ...
    demos_registry = {
        ...
        "feedback_writing": demo_feedback_writing,
        "self_reflection": demo_self_reflection,
        "goal_setting": demo_goal_setting,
        "one_on_one": demo_one_on_one,
        "feedback_request": demo_feedback_request,
        "shoutout": demo_shoutout,
        "calibration": demo_calibration,
        "competency": demo_competency,
        "summary": lambda _p: print_summary(),
    }

    if args.list_demos:
        print_banner()
        print("Available demos:")
        for key in demos_registry.keys():
            print(f"  - {key}")
        return
    ...
    selected = list(demos_registry.keys()) if args.all else args.demo
    if not selected:
        print("Tip: use --list-demos to see available demos, or --all to run everything.")
        return

    for name in selected:
        fn = demos_registry.get(name)
        if not fn:
            print(f"Unknown demo: {name}. Use --list-demos to see choices.")
            continue
        fn(platform)

    if args.summary and "summary" not in selected:
        print_summary()
